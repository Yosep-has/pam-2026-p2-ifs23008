...
class TodoView(private val todoService: ITodoService) {
    ...
    fun updateTodo() {
        ...
        val strId = InputUtil.input("ID Todo (x Jika Batal)")
        if (strId == "x") return

        val newTitle = InputUtil.input("Judul Baru")
        todoService.updateTodo(strId.toInt(), newTitle)
    }
    ...
    fun sortTodo() {
        ...
        println("1. Berdasarkan Judul")
        println("2. Berdasarkan Status")

        val input = InputUtil.input("Pilih")
        todoService.sortTodo(input.toInt())
    }
